...
\NewDocumentCommand{\sesa}{m m O{} O{} O{cm}}{%
    \href{https://manuel.sesamath.net/numerique/index.php?ouvrage=#5#1_#2&page_gauche=#4}{%
    \ifstrequal{#5}{cm}{Cahier}{Manuel} Sésamath #1e #2 \ifNotNull{#4}{(\ifNotNull{#3}{#3 }p.#4)}
    }
}
...
\NewDocumentCommand{\wiki}{m O{}}{%
    ...
    {\def\iconPath{}%
    \icon{wikipedia} \reverseSnakeCase{#1}%
...
\def\ca{%
    ...
    {Course aux nombres ( \reverseKebabCase{\caPrefix})}%
}
...
\newcommand{\cmdIconLink}[4]{% 1 text , 2 : color , 3 : icon , 4 : link
    \expandafter\NewDocumentCommand\csname cmd#1\endcsname{O{}}
    {%
        {\def\iconPath{}\def\iconSize{15pt}%
        \href{#4/##1}{\shl{capytale-2}{#2}{\icon{#3} ##1}}
    }%
}}
...
\definecolor{mathalea}{HTML}{216D9A} % #216D9A
\cmdIconLink{MathAlea}{mathalea}{mathalea}{https://coopmaths.fr/alea/?uuid=b87a5&id=}

\NewDocumentCommand{\mal}{m O{MathAlea}}{
    {\def\iconPath{}\def\iconSize{15pt}%
        \href{https://coopmaths.fr/alea/?uuid=#1&v=eleve&i=1}{\shl{capytale-2}{mathalea}{\icon{mathalea} #2}}
    }
}

% uuid=68cda

% https://coopmaths.fr/alea/?uuid=1d078&id=4C33-0&n=4&d=10&s=2&s2=3&s3=1&s4=3&i=1&cd=1&alea=48pq&v=eleve&es=0111001

% http://trucsmaths.free.fr/etymologie.htm

\def\ym{\href{https://www.maths-et-tiques.fr/}{Yvan Monka}}
\newcommand{\dym}[1]{\def\ym{\href{#1}{Yvan Monka}}}

\newcommand{\yvan}[1]{\def\ym{\href{https://www.maths-et-tiques.fr/telech/#1}{Yvan Monka (#1)}}}

\NewDocumentCommand{\met}{mO{}}{%
    \href{https://www.maths-et-tiques.fr/telech/#1.pdf\#page=#2}
    {maths et tiques (#1 \ifNotNull{#2}{p.#2})}
}

\def\cwr{\href{https://college-willy-ronis.fr/}{College Willy Ronis}}
\newcommand{\dcwr}[1]{\def\cwr{\href{#1}{College Willy Ronis}}}

% https://coopmaths.fr/alea/?uuid=b87a5&id=4L10b&alea=ap5e
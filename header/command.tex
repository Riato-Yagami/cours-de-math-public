...
\NewDocumentCommand{\drawPoint}{mmm O{(0,0)}}{
...
\NewDocumentCommand{\drawTick}{m O{}}{
    \draw[line width = 0.75mm, gradeColor] (#1, 0.08) -- (#1, -0.08);
    \draw[line width = 0.75mm, gradeColor] (#1 - 0.08, 0) -- (#1 + 0.08, 0);
    \node[gradeColor] at (#1, -0.3) {#2};
}
...
\NewDocumentCommand{\answerFill}{O{Réponse}}{%
    \answerSec{\remaininglines}[#1]%
    \newpage%
}

\NewDocumentCommand{\answerSec}{m O{Réponse}}{\vspace{0.25cm}%
    #2 :%
    \quad \dottedLines{#1}%
}

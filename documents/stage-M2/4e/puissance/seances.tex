...
\slide{}{
    \pr{}{
        Ces 3 remarque sont enfaite une propriétés pour les puissances de tous nombres $x$.
        \begin{align*}
            x^a\times x^b = x^{a+b}
            \iet
            \frac{x^a}{x^b} = x^{a-b}
...
\slide{}{
    \ssubsec
    \df{}{
        La notation scientifique d'un nombre est la seul écriture de la forme:
        \begin{equation*}
            \palt{2}{a \times 10^n}
        \end{equation*}
        $a$ est un nombre qui s'écrit avec un seul chiffre autre que $0$ avant la virgule.
    }
}
...
\slide{}{
    \df{}{
        Un \textbf{ordre de grandeur} est un nombre qui représente de façon simplifiée,
        mais approximative la mesure d'une grandeur.
        C'est la puissance de 10 la plus proche de la grandeur étudiée.
    }
}

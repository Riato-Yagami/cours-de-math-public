...
\demo{}{
    \begin{center}
        ...
        \begin{tikzpicture}
            ...
            \tkzDefPoint(\b+\ys,\h){G}
            \tkzDefPoint(\b,0){H}
            \draw[fill = blue, opacity = 0.2] (A) -- (B) -- (F) -- cycle;
            \draw[fill = blue, opacity = 0.2] (C) -- (D) -- (G) -- cycle;
...
\demo{avec une intégrale}{%
    ...
    \dividePage{%
        \ctikz{
            ...
            \tkzDefPoint(\x,0){X}
            \tkzDefPoint(0,\y){Y}
            \draw[fill = blue, fill opacity = 0.2] (A) arc (0:90:\r) -- (O) -- cycle ;
            \draw[thick] (O) -- (P);
...
\section{Propriétés sur les aires et périmètre}

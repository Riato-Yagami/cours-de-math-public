...
\pr{Perimètre de polygones particuliers}{
    \begin{itemize}[wide=0pt, leftmargin=*]
        \item Carré ou losange de coté $c$: $\Per = 4c$
        \item Rectangle de longueur $L$ et largeur $l$: $\Per = 2L +2l$
        \item Triangle équilatéral de coté $c$: $\Per = 3c$
    \end{itemize}
}
    ...
    \dividePage{
        \begin{center}
            \begin{tikzpicture}[scale=1]
                ...
                \tkzDefPointBy[projection=onto A--B](C) \tkzGetPoint{H}
                \tkzLabelPoints[below](H)
                \tkzFillPolygon[blue, opacity=0.2](A,C,D)
                \tkzFillPolygon[red, opacity=0.2](B,C,D)
                \draw[very thick] (A) -- (B) -- (C) -- cycle;
...
\demo{}{
    ...
    a une aire $\B_t = \B \frac{(h-t)^2}{h^2}$ (découle de Thalès).
    On a alors:
    \begin{align*}
        \V &= \int_{0}^{h} \B_t dt\\ 
...
\df{Aire sous la courbe}{
    L'\textbf{aire sous la courbe} \C sur $[a,b]$ est l'aire du domaine délimité par;
...
\section{Determinants dans le plan}
...
\demo{}{
    ...
    \begin{center}
        \begin{tikzpicture}
            \axis{4}
            \defvectors
            \fill[blue, opacity=0.2] (O) -- (U) -- (UV) -- (V) -- cycle;
            \drawvectors
        \end{tikzpicture}
        % \begin{tikzpicture}
        %     \axis{4}
        ...
        \begin{tikzpicture}
            ...
            \fill[blue, opacity=0.2] (Ob) -- (Ot) -- (Oq) -- (Ub) -- (UVb) -- (Vb) -- cycle;
            \drawvectors
        \end{tikzpicture}
    \end{center}

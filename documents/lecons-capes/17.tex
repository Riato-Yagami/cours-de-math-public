...
\df{Aire latérales}{
    L'\textbf{aire latérale} $\mathcal{A}$ d'un solide est la surface délimitant privée de ses bases.
}
...
\df{Volume}{
    Le \textbf{volume} $\mathcal{V}$ d'un solide est ca mesure d'espace, en comparaison \mt.
}
...
\pr{Aire et volume du pavé droit}{
    ...
    on a:
    \begin{align*}
        \mathcal{A} &= h \times 2 (L+l)\\
        \mathcal{V} &= h \times L \times l
    \end{align*}
}
...
\pr{Aire et volume du prisme et du cylindre}{
    ...
    de base d'aire $\mathcal{B}$ et périmètre $\mathcal{P}$,
    on a:
    \begin{align*}
        \mathcal{A} &= \mathcal{P} \times h\\
        \mathcal{V} &= \mathcal{B} \times h
    \end{align*}
}

\subsection{Pyramide et cone}
...
\demo{}{
    ...
    a une aire $\mathcal{B}_t = \mathcal{B} \frac{(h-t)^2}{h^2}$ (découle de Thalès).
    On a alors:
    \begin{align*}
        \mathcal{V} &= \int_{0}^{h} \mathcal{B}_t dt\\ 
        &= \int_{0}^{h} \mathcal{B} \frac{(h-t)^2}{h^2} dt = \frac{\mathcal{B}}{h^2} \int_{0}^{h} (h-t)^2 dt\\
        &= \frac{\mathcal{B}}{h^2} \int_{h}^{0} u^2 (-du) \textrm{par changement de variable} u = h-t \et \alors du = -dt\\
        &= \frac{\mathcal{B}}{h^2} \int_{0}^{h} u^2 du = \frac{\mathcal{B}}{h^2} \times h^3 = \frac{\mathcal{B} \times h}{3}
    \end{align*}
...
\pr{Aire et volume d'une sphère}{
    L'aire $\mathcal{A}$ et le volume $\mathcal{V}$ d'une shère de rayon $R$ sont donnée par le formules :
    \begin{align*}
        \mathcal{A} = 4\pi R^2\\
        \mathcal{V} = \frac{4\pi R^3}{3}
    \end{align*}
    % \begin{tikzpicture}
    %     \def\r{3}
...
\demo{}{
    ...
    \begin{align*}
        dxdydz = d\mathcal{V} &= r dr d\theta dz\\
        \ior z &= r\cos(\theta)\\
        \idou dz &= r\sin(\varphi) d\varphi\\
        \ialors d\mathcal{V} &= r^2 \sin(\varphi) dr d\theta d\varphi\\
        \ialors \mathcal{A} &= \int_{0}^{2\pi} \int_{0}^{\pi} R^2 \sin(\varphi) d\varphi d\theta\\
        &= \int_{0}^{2\pi} 2R^2 d\theta
        = 4\pi R^2\\
        \iet \mathcal{V} &= \int_{0}^{R} 4\pi r^2 dr
        = \frac{4\pi R^3}{3}
    \end{align*}
}

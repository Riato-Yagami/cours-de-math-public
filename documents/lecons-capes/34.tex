...
[\item Continuités et dérivation
...
\demo{}{
    ...
    \begin{align*}
        \delta = \log_b(x)
        \ialors x^\delta = b
        \ialors \log_a(x^\delta) = \log_a(b)
        \ialors \delta \log_a(x) = \log_a(b)
        \ialors \log_b(x) = \frac{\log_b(a)}{\delta}
...
\demo{}{
    \rmdr{Dérivation de fonction composées}{
...
\pr{Dérivation composées du \logn}{
...
\demo{}{
    ...
    \begin{align*}
        f(x) &= \ln(x)-2\sqrt{x}
        \ialors f'(x) &= \frac{1}{x} - \frac{1}{\sqrt{x}} \car f \; C^1 \textrm{ sur } [1,+\infty[
...
\section{Méthode d'approximations}
...
\tice{Algorithme de Briggs}{Python}{
    \lstinputlisting[language=Python]{documents/lecons-capes/ressources/algo_briggs.py}
}
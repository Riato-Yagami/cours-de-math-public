...
% %Tapuscrit : Denis VergÃ¨s
% %Relecture : FranÃ§ois Hache
...
% %L'usage de calculatrice sans mémoire \og type collÃ¨ge \fg{} est autorisé}
% %
% %\vspace{0,5cm}
% %\framebox[16cm]{
% %\begin{minipage}{15cm}
...
% %\end{center}
% % Toutes les réponses doivent Ãªtre justifiées, sauf si une indication contraire est donnée.\\
% %Pour chaque question, si le travail n'est pas terminé, laisser tout de mÃªme une trace de la 
...
% %Déterminer la valeur de $b$ telle que le périmÃ¨tre du rectangle ci-contre soit égal à 25.
% Le rectangle a une largeur de 4,5 et une longueur de 3b + 2,9.

% Son périmÃ¨tre est égal à : $2(4,5 + 3b + 2,9) =$
...
% On sait que $V = \dfrac13 \times B \times h$, avec $B = 3 \times 4 = 12$ et $h = 6$, d'oÃ¹ :
...
% $x \times 1,12 = \np{20692}$, d'oÃ¹ en multipliant chaque membre par $\dfrac{1}{1,12}$, \quad $x = \dfrac{\np{20692}}{1,12} = \np{18475}$.
...
% Un poteau électrique vertical [BC] de $5,2$ m de haut est retenu par un cÃ¢ble métallique [AC] comme montré sur le schéma 1 qui n'est pas en vraie grandeur.
% \end{minipage}\hfill
...
% \begin{enumerate}
% \item %Montrer que la longueur du cÃ¢ble [AC] est égale à $6,5$~m.
% le théorÃ¨me de Pythagore appliqué au triangle ABC rectangle en B, s'écrit :
...
% \item %Calculer la mesure de l'angle $\widehat{\text{ACB}}$ au degré prÃ¨s.
...
% La mesure de l'angle $\widehat{\text{ACB}}$ est $37\degres$ au degré prÃ¨s.
% \end{enumerate}

% Deux araignées se trouvant au sommet du poteau (point C) décident de rejoindre le bas du cÃ¢ble (point A) par deux chemins différents.

% \begin{enumerate}[resume]
% \item %La premiÃ¨re araignée se déplace le long du cÃ¢ble [AC] à une vitesse de $0,2$~m/s. 

% %Vérifier qu'il lui faut 32,5 secondes pour atteindre le bas du cÃ¢ble.
% On a $v = \dfrac dt$, avec $v = 0,2$ et $d = \text{CA} = 6,5$.

% Donc $0,2 = \dfrac{6,5}{t}$, d'oÃ¹ $0,2t = 6,5$ et $t = \dfrac{6,5}{0,2} = 6,5 \times 5 = 32,5$~(s).
% \item La deuxiÃ¨me araignée décide de parcourir le chemin CFHA indiqué en pointillés sur le schéma 2 (qui n'est pas en vraie grandeur) : elle suit le morceau de cÃ¢ble [CF] en marchant, puis descend verticalement le long de [FH] grÃ¢ce à son fil et enfin marche sur le sol le long de [HA].

% %Calculer les longueurs FH et HA.
% Les droites (FH) et (CB) étant toutes les deux perpendiculaires à la droite (AB) sont parallÃ¨les.

% $\bullet~~$D'aprÃ¨s le théorÃ¨me de ThalÃ¨s : $\dfrac{\text{FH}}{\text{BC}} = \dfrac{\text{AF}}{\text{AC}}$, soit $\dfrac{\text{FH}}{5,2} = \dfrac{4}{6,5}$, d'oÃ¹ en multipliant chaque membre par 6,5 : $\text{FH} = \dfrac{4 \times 5,2}{6,5} = \dfrac{4 \times 52}{65} = \dfrac{4 \times 4 \times 13}{5 \times 13} = \dfrac{16}{5} = \dfrac{32}{10} = 3,2$~(m).

% $\bullet~~$On a de mÃªme toujours d'aprÃ¨s ThalÃ¨s : $\dfrac{\text{AH}}{\text{AB}} = \dfrac{\text{AF}}{\text{AC}}$, soit $\dfrac{\text{AH}}{3,9} = \dfrac{4}{6,5}$, d'oÃ¹ en multipliant chaque membre par 3,9 : $\text{AH} = \dfrac{3,9 \times 4}{6,5} = \dfrac{39 \times 4}{65} = \dfrac{3 \times 13 \times 4}{5 \times 13} = \dfrac{12}{5} = \dfrac{24}{10} = 2,4$~(m).
% % \begin{center}
% % \psset{unit=1cm,arrowsize=2pt 3}
...
% \item %La deuxiÃ¨me araignée marche à une vitesse de 0,2 m/s le long des segments [CF] et [HA] et descend le long du segment [FH] à une vitesse de 0,8 m/s.

% %Laquelle des deux araignées met le moins de temps à arriver en A ?
% De $v = \dfrac{d}{t}$, on tire $d = v \times t$ et $t = \dfrac{d}{v}$.

% La deuxiÃ¨me araignée parcourt CF + HA $= (6,5 - 4)  + 2,4 = 4,9$~(m) à la vitesse de 0,2~(m/s).
...
%
%On considÃ¨re les deux scripts suivants:
%
%\begin{center}
...
% 	\end{enumerate}
% \item On considÃ¨re le programme de calcul suivant:
% \begin{center}
...
% La réponse, affirmative ou négative, devra Ãªtre argumentée.

% On a vu à la question précédente que la fréquence du score 10 points est égale à $\dfrac{1}{36}$.

% Donc pour \np{1000} joueurs il y aura à peu prÃ¨s :

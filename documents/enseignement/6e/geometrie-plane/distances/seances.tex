...
\def\exogrid{
    \ctikz[0.75]{
            ...
            \draw[very thick, gradeColor] (2,8) -- (4,8) node[midway, above=0.25cm] {$1\;\centi\meter$};
...
\slide{exo}{
    ...
    \exo{}{En s'apuyant sur la figure ci-dessous, compléter les phrases suivantes :
        \begin{enumerate}
            \item Le point $A$ est situé à \bawsr{$2$ }$\centi\meter$ de la droite $(d)$.
            \item La distance du point $B$ à la droite \bawsr{$(d)$} vaut $2 \centi\meter$.
            \item Le point $B$ est à la distance $BF$ de la droite \bawsr{$(d')$}.
            \item La distance du point $A$ à la droite $(d'')$ est \bawsr{$AE$}.
            \item Le point $D$ est situé à \bawsr{$0$} $\centi\meter$ de la droite $(d')$.
...
\scn{Découvrire la définition d'un cercle}
...
\scn{Découvrire la définition d'un disque}

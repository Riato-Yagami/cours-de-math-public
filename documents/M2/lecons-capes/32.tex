...
\begin{equation*}
    f'(x_0) = \lm{h}{0}\dfrac{f(x_0+h)-f(x_0)}{h}
\end{equation*}

\rmq{}
On peut également définir le nombre dérivé de la façon suivante:
\begin{equation*}
    f'(x_0) = \lm{x}{x_0}\dfrac{f(x)-f(x_0)}{x-x_0}
\end{equation*}
...
\begin{center}
    \begin{tabular}{l | c | c}
        ...
        & $\m{R}$ 
        & $nx^{n-1}$ \\
        $x \mapsto \dfrac{1}{x^n}$ avec $n\in\m{N}$
        & $\m{R^*}$ 
        & $-\dfrac{n}{x^{n+1}}$ \\
        $x \mapsto \sqrt{x}$
        & $\m{R_+^*}$ 
...
\begin{center}
    \begin{tabular}{c | c}
        ...
        $ku$ & $ku'$\\
        $uv$ & $u'v+uv'$\\
        $\dfrac{1}{u}$ avec $u$ non nul sur $\m{I}$ & -$\dfrac{u'}{u^2}$\\
        $\dfrac{u}{v}$ avec $v$ non nul sur $\m{I}$ & -$\dfrac{u'v-uv'}{v^2}$\\

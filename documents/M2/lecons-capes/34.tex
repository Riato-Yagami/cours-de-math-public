...
\demo{}{
    ...
    \begin{align*}
        ...
        \idou \log_b(x) &= \dfrac{\log_a(x)}{\log_a(b)}
    \end{align*}
...
\pr{Dérivations composées avec le \logn}{
    ...
    Alors $\ln(u)$ est dérivable et $\ln'(u) = \dfrac{u'}{u}$.
}
...
\demo{}{
    ...
    \begin{align*}
        ...
        \ior f'(x) &= \dfrac{1}{x} \et g'(x) = u'(x)
        \ialors (f \circ g)'(x) &= u'(x) \times \dfrac{1}{u(x)}
        \idou \ln'(u) &= \dfrac{u'}{u}
    \end{align*}
...
\def\lnx{\dfrac{\ln(x)}{x}}
\def\lnxn{\dfrac{\ln(x)}{x^n}}
...
\demo{}{
    ...
    \begin{align*}
        f(x) &= \ln(x)-2\sqrt{x}
        \ialors f'(x) &= \dfrac{1}{x} - \dfrac{1}{\sqrt{x}}\;\;  (f \textrm{ de classe } C^1 \textrm{ sur } [1,+\infty[)
        \ior x &\geq \sqrt{x} > 0 \car x \geq 1
        \ialors f'(x) &\leq 0
        \ior f \textrm{ décroissante sur } &[1,+\infty[ \et f(1) = -2 \leq 0
        \ialors \ln(x) &\leq 2 \sqrt{x}
        \ialors 0 \leq \lnx &\leq \dfrac{2}{\sqrt{x}}
...
\demo{}{
    ...
    on a:
    \begin{align*}
        \lnxn &= \dfrac{1}{x^{n-1}} \times \lnx
        \ior \limP \dfrac{1}{x^{n-1}} &= 0 \et \limP \lnx = 0
        \idou \limP \lnxn &= 0
    \end{align*}
}[\bib]
...
\demo{}{
    ...
    on a:
    \begin{align*}
        \xln &= \dfrac{1}{u} \ln\left(\dfrac{1}{u}\right) = -\dfrac{\ln(u)}{u} \avec u = \dfrac{1}{x}
        \ialors \limO \xln &= \lim\limits_{u\to+\infty} -\dfrac{\ln(u)}{u} = 0
    \end{align*}
}[\bib]
...
\demo{}{
    ...
    on a:
    \begin{align*}
        \ln\left(\dfrac{1}{a} \times a\right) &= \ln(1) = 0
        \iet \ln\left(\dfrac{1}{a} \times a\right) &= \ln\left(\dfrac{1}{a}\right) + \ln(a)
        \idou \ln\left(\dfrac{1}{a}\right) &= -\ln(a)
    \end{align*}
}[\hyp]
...
\mthd{Algorithme de Briggs}{
    ...
    \begin{algorithm}
        ...
        \KwRes{$\ln(a)$ a $10^{-n}$ près ($\rightarrow 2^i \times (a^{\dfrac{1}{2^i}} - 1)$)}
        $i \leftarrow 0$\;
        $u \leftarrow a$\;
        \Tq{$|u-1| \geq 10^{-n}$}{
            $i \leftarrow i+1$\;
...
\demo{}{
    ...
    \begin{align*}
        ...
        \ialors \ln(y) &= 2^i(y^{\dfrac{1}{2^i}} - 1) + \dl{y \rightarrow 1^{2^i} = 1} (y^{\dfrac{1}{2^i}}-1) \textrm{ changement de variable } y = z^{2^i}
        \ior \lim\limits_{i\rightarrow+\infty} y^{\dfrac{1}{2^i}} &= 1
        \idou \ln(y) &\approx 2^i(y^{\dfrac{1}{2^i}} - 1) \textrm{ quand $i$ grand}
        % \ialors \ln(x) &\approx x-1 \textrm{ quand $x$ est proche de 1.}
        % \ior 2^i\ln(x) &= \ln(x^{2^i})
        % \ialors 2^i\ln(y^{\dfrac{1}{2^i}}) &= \ln(y^{\dfrac{2^i}{2^i}}) = \ln(y) \textrm{ par changement de variable } y = x^{2^i}
        % \ialors \ln(y) &\approx 2^i \times (y^{\dfrac{1}{2^i}} - 1) \textrm{ quand $i$ grand}  
        % \ior u &= a^{\dfrac{1}{2^i}}
    \end{align*}
...
\mthd{Algorithme de Brouncker}{
    ...
    \begin{equation*}
        \ln(2) = \sum_{k=0}^{+\infty}\dfrac{1}{(2k+1)(2k+2)}
    \end{equation*}

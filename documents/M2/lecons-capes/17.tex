...
\demo{}{
    ...
    On a:
    \begin{align*}
        a_n &= \dfrac{360}{2n} = \dfrac{180}{n}\\
        \ior \sin(a_n) &= \dfrac{\textrm{opposé}}{\textrm{hypothénus}}
        = \dfrac{i_n / 2}{\textrm{rayon}}
        = \dfrac{i_n / 2}{\textrm{1}}
        = \dfrac{i_n}{2}\\
        \iet \tan(a_n) &= \dfrac{\textrm{opposé}}{\textrm{adjacent}}
        = \dfrac{c_n / 2}{\textrm{rayon}}
        = \dfrac{c_n / 2}{\textrm{1}}
        = \dfrac{c_n}{2}
        \ior U_n &= n \times i_n = 2 n \sin(a_n)
        \iet V_n &= n \times c_n = 2 n \tan(a_n)
        \ior \dfrac{U_n}{2} &\leqslant \pi \leqslant \dfrac{V_n}{2}
        \ialors n \sin(\dfrac{180}{n}) &\leqslant \pi \leqslant n \tan(\dfrac{180}{n})
    \end{align*}
...
\demo{}{
    L'arc est une section $\dfrac{\theta}{2\pi}$ de la circonférence.\\
    Alors la longueur d'arc est: $2\pi r \times \dfrac{\theta}{2\pi} = r \theta$.
}
...
\pr{Découpage et recollage de rectangle à un carré de même aire}{
    ...
    Si $L \geqslant \dfrac{\sqrt{lL}}{2}$ alors on peut découper et recoller le rectangle pour trouver un carré de même aire.\\
...
\pr{Aires du triangle et du parallélogramme}{
    \begin{itemize}[wide=0pt, leftmargin=*]
        \item Triangle de base $b$ et hauteur $h$ : $\A = \dfrac{b \times h}{2}$.
...
\demo{d'Archimède}{
    ...
    \begin{align*}
        \A &= l \times L\\
        \ior 2L = \C &\et L = r\\
        \ialors \A &= \dfrac{\C}{2} \times r
        = \dfrac{2\pi r}{2} \times r
        = \pi r^2
    \end{align*}
...
\demo{avec une intégrale}{%
    ...
    \dividePage{%
        ...
        \begin{align*}
            ...
            % \ior \sqrt{r^2-x^2} &= r \sqrt{1 - \dfrac{x^2}{r^2}}\\
            % \shortintertext{et par changement de variable} u = \dfrac{x}{r} \alors dx = rdu\\
            % \iona 
        \end{align*}
    }
...
\lem{des bases}{
    ...
    \begin{align*}
        \dfrac{\A_{ABM}}{\A_{ACM}} = \dfrac{BM}{CM}
    \end{align*}
}
...
\demo{}{
    ...
    On a:
    \begin{align*}
        \dfrac{\A_{ABM}}{\A_{ACM}} = \dfrac{\dfrac{BM \times h}{2}}{\dfrac{CM \times h}{2}} = \dfrac{BM}{CM}
    \end{align*}
...
\demo{}{
    ...
    \dividePage{
        ...
        \begin{align*}
            \dfrac{\A_{ACM}}{\A_{ABC}} = \dfrac{AM}{AB}
            \iet \dfrac{\A_{ABN}}{\A_{ABC}} = \dfrac{AN}{AC}
            \ior \A_{ACM} = \A_{ABN}
            \ialors \dfrac{AM}{AB} = \dfrac{AN}{AC}
        \end{align*}
    }[0.45]
}[\href{https://le-castillon-le-pieux.college.ac-normandie.fr/IMG/pdf/Thales_-_Demonstrations.pdf}{Collège Le Castillon}]
...
\demo{}{
    ...
    a une aire $\B_t = \B \dfrac{(h-t)^2}{h^2}$ (homothétie de rapport $\dfrac{(h-t)}{h}$).
    On a alors:
    \begin{align*}
        \V &= \int_{0}^{h} \B_t dt\\ 
        &= \int_{0}^{h} \B \dfrac{(h-t)^2}{h^2} dt = \dfrac{\B}{h^2} \int_{0}^{h} (h-t)^2 dt\\
        &= \dfrac{\B}{h^2} \int_{h}^{0} u^2 (-du) \textrm{par changement de variable} u = h-t \et \alors du = -dt\\
        &= \dfrac{\B}{h^2} \int_{0}^{h} u^2 du = \dfrac{\B}{h^2} \times h^3 = \dfrac{\B \times h}{3}
    \end{align*}
